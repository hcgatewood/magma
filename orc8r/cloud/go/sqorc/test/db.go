...
func GetSharedMemoryDB() (*sql.DB, error) {
	var err error
	once.Do(func() { instance, err = sqorc.Open(storage.SQLDriver, ":memory:") })
	return instance, err
}
...
func setDialect(driver string) {
	...
	switch driver {
	case sqorc.PostgresDriver:
		_ = os.Setenv("SQL_DIALECT", sqorc.PostgresDialect)
	case sqorc.MariaDriver:
		_ = os.Setenv("SQL_DIALECT", sqorc.MariaDialect)
	}
}
