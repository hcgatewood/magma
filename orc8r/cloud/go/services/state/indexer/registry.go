...
// File service_registry.go provides an indexer registry by forwarding calls to
...
func GetIndexers() ([]Indexer, error) {
	services, err := service_registry.FindServices(orc8r.StateIndexerLabel)
	if err != nil {
		return []Indexer{}, err
	}
...
// service metadata.
func getIndexer(serviceName string) (Indexer, error) {
	versionVal, err := service_registry.GetAnnotation(serviceName, orc8r.StateIndexerVersionAnnotation)
	if err != nil {
		return nil, err
	}
	...
	types, err := service_registry.GetAnnotationList(serviceName, orc8r.StateIndexerTypesAnnotation)
	if err != nil {
		return nil, err
	}
...
func DeregisterAllForTest(t *testing.T) {
	...
	service_registry.RemoveServicesWithLabel(orc8r.StateIndexerLabel)
}
